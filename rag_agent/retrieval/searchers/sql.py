...
请只输出'是'或'否'，不要添加任何其他解释。
    ...
    def _generate_sql_from_query(self, query: str) -> str:
        ...
        - report_period (TEXT): 报告期，格式如 "2025-Q1", "2024-H1", "2024-FY"
        - metric_name (TEXT): 财务指标名称，如 "营业收入", "净利润", "ROE"
        - metric_value (REAL): 财务指标数值
        - unit (TEXT): 单位，如 "元", "%", "百万元"
        
        【重要】银行财务指标命名规则：
        - 不同银行可能使用不同的指标名称，例如：
          * 招商银行："净利息收入", "净手续费及佣金收入", "其他净收入小计"
          * 中信银行："利息净收入", "手续费及佣金净收入", "投资收益", "汇兑收益"
        - **非利息净收入**在数据库中没有直接存储，需要查询以下相关指标：
          * 手续费及佣金相关："手续费及佣金净收入", "净手续费及佣金收入"
          * 其他收入项："投资收益", "公允价值变动损益", "汇兑收益", "其他净收入小计"
          * 或通过计算：营业收入 - 利息净收入
        
        【查询策略】当遇到数据库中可能不存在的指标名时：
        1. 使用 LIKE 进行模糊匹配查找相关指标
        2. 查询该指标的组成部分（如非利息净收入 = 手续费 + 投资收益 + 其他）
        3. 先查询所有可能相关的指标，再在结果中筛选
        """
...
2. **精准条件**：只根据用户明确提到的条件生成WHERE子句
3. **支持复杂查询**：合理使用 ORDER BY, SUM(), AVG(), GROUP BY 等聚合和排序操作
4. **模糊匹配策略**：
   - 对于直接存在的指标（如"营业收入"、"净利润"），使用精确匹配或LIKE模糊匹配
   - 对于复合指标（如"非利息净收入"），查询其组成部分：
     * 手续费相关：WHERE metric_name LIKE '%手续费%' OR metric_name LIKE '%佣金%'
     * 投资收益相关：WHERE metric_name LIKE '%投资收益%' OR metric_name LIKE '%公允价值%'
     * 其他收入：WHERE metric_name LIKE '%其他%收入%' OR metric_name LIKE '%汇兑%'
   - 对于不确定的指标，使用宽泛的LIKE条件查询所有可能相关的指标
5. **纯净输出**：只输出SQL语句，不要添加任何解释、标记或代码块符号

【特别提示】
- 当用户询问"非利息净收入"时，应查询：手续费及佣金、投资收益、汇兑收益、其他净收入等相关指标
- 使用 OR 连接多个 LIKE 条件以覆盖不同银行的命名差异
- 确保结果包含 company_name, report_period, metric_name, metric_value, unit 字段
